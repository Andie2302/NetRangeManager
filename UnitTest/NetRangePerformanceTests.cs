...
public class NetRangePerformanceTests
{
    ...
    public void Performance_ContainsCheck_FastForManyAddresses()
    {
        ...
        var addresses = new[] { IPAddress.Parse ( "192.168.1.1" ) , IPAddress.Parse ( "192.168.100.100" ) , IPAddress.Parse ( "192.168.255.255" ) , IPAddress.Parse ( "10.0.0.1" ) , };
        var stopwatch = System.Diagnostics.Stopwatch.StartNew();

        for ( int i = 0 ; i < 10000 ; i++ ) {
            foreach ( var addr in addresses ) { range.Contains ( addr ); }
        }
